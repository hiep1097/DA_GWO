...
public class f_SimRMC {
    ...
    public ArrayList<RMCTruckSchedule> lstRMCTruckSchedule = new ArrayList<RMCTruckSchedule>();    // Thông tin chi tiết 1 lần đỗ bên tông của một xe
    public ArrayList<ScheduleTruck> lstScheduleTrucks = new ArrayList<ScheduleTruck>();         // Thông tin lịch đỗ bê tông của một xe
            ...
            try (Scanner scan = new Scanner(f)) {
                ...
                for(int i = 0; i < m; i++){
                    if(scan.hasNextLine()){
                        ...
                        s.PT = data[3];
                        s.ABD = Integer.parseInt(data[4]);
                        s.ABT = Integer.parseInt(data[5]);
                ...
                for(int i = 0; i < numberOfStation; i++){
                    ...
                    line = readLineFromFile(scan);
                    data = line.split("\\:");
                    rmcStation.timeMD = Integer.parseInt(data[1]);
...
//                s.calTimeTruckMove(vOfGo, vOfBack);
    ...
    public void initRMC(){
        ...
        //Thực hiện khởi tạo quần thể sắp lịch bàn đầu từ kết quả của thuật toán GWO thông qua x_rand[]
        int t = 0;
//        Random r = new Random();
        for(Site s : lstSite){
...
//                rmcSite.value = r.nextInt(1000);

                arrSite.add(rmcSite);
            }
        }

        // Dựa và giá trị value của dãy Random/Thuật toán GWO, sắp sếp lại thứ tự đỗ tại các công trường
    ...
    public void initScheduleTruck(){
        ...
        for(RMCStation r : lstRMCStation) {
            ...
            for (int i = 0; i < r.c; i++) {
                ...
                int deltaTimeOutputOfTruck = (i < lstRMCTruckSchedule.size()) ? ((i == 0) ? 0 : (lstRMCTruckSchedule.get(i).delivery * r.timeMD)) : 0;
    ...
    public void ExecuteAlgorithm(){
        ...
        for(RMCTruckSchedule rts : lstRMCTruckSchedule){
            ...
            rts.calMD_and_CD_RMC(station.timeMD);
            ...
            if(truckTBB != 0 && (truckTBB + rts.MD) > SDT)
                SDT = truckTBB + rts.MD;
